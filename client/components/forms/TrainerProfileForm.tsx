...
export default function TrainerProfileForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const saved = await api.upsertTrainerProfile(
        body,
        initial ? "PUT" : "POST",
      );
      onSaved?.(saved);
    ...
    <form onSubmit={handleSubmit} className="grid gap-4">
      ...
      <div className="grid md:grid-cols-2 gap-3">
        <div className="grid gap-1.5">
          ...
          <Input name="certifications" defaultValue={initial?.certifications} />
        </div>
        <div className="grid gap-1.5">
          ...
          <Input
            ...
            defaultValue={initial?.training_type}
          />
        </div>
      </div>
      <div className="grid md:grid-cols-2 gap-3">
        <div className="grid gap-1.5">
          ...
          <Input
            ...
            min={0}
            step="0.01"
            defaultValue={initial?.price_per_session}
          />
        </div>
