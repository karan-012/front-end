...
export default function ClientProfileForm({ initial, onSaved }: { initial?: any; onSaved?: (p: any) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const body = {
      ...
      preferred_training_type: String(form.get("preferred_training_type") || ""),
      budget: Number(form.get("budget") || 0),
    };
    setError("");
    setLoading(true);
    try {
      const saved = await api.upsertClientProfile(body, initial ? "PUT" : "POST");
      onSaved?.(saved);
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4">
      ...
      <div className="grid md:grid-cols-2 gap-3">
        <div className="grid gap-1.5"><label className="text-sm font-medium">Fitness goal</label><Input name="fitness_goal" defaultValue={initial?.fitness_goal} /></div>
        <div className="grid gap-1.5"><label className="text-sm font-medium">Preferred training type</label><Input name="preferred_training_type" placeholder="Online / In-person" defaultValue={initial?.preferred_training_type} /></div>
      </div>
      <div className="grid md:grid-cols-2 gap-3">
        <div className="grid gap-1.5"><label className="text-sm font-medium">Budget</label><Input name="budget" type="number" min={0} step="0.01" defaultValue={initial?.budget} /></div>
