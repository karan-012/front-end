  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-gradient-to-r from-[hsl(var(--brand-start))] to-[hsl(var(--brand-end))] text-white w-full">
          Book session
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Choose a date and time to request a session.
          </DialogDescription>
        </DialogHeader>
        {error && (
          <div className="mb-3 rounded border border-destructive/50 bg-destructive/10 px-3 py-2 text-sm text-destructive-foreground">
            {error}
          </div>
        )}
